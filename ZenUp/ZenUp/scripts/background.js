...
async function updateRules(enable) {
  try {
    ...
    if (!enable || !blockedSites || blockedSites.length === 0) {
      console.log('清除所有规则');
    ...
    // 如果 blocklist 里只有 *，则只添加一条全局阻止规则
    if (blockedSites.length === 1 && blockedSites[0] === '*') {
      console.log('阻止所有网站模式，仅添加全局阻止规则');
      await chrome.declarativeNetRequest.updateDynamicRules({
      ...
      console.log('当前生效的规则:', updatedRules);
      return;
    }
    ...
    console.log('准备添加的新规则:', JSON.stringify(newRules, null, 2));
